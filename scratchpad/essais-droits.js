...
const ESSAIS = [
  ...
  { nom: 'Intervention SANS technicien : un DR rattaché à une équipe la trouve (recherche) pour l’affecter', retire: { planifDeplacer: false },
    ...
    mesure: `window.__vu` },
  /* relecture v739 : l'option « à affecter » ne doit PAS ouvrir les clients (la cascade bloquante) */
  { nom: 'Technicien planificateur (sans « Tout voir ») : il trouve l’intervention à affecter…', type: 'unique', attendu: 'passe', retire: { voirTout: false },
    base: `function(){ db.interventions.push(Object.assign({},db.interventions.find(x=>x.id==='iZ'),{id:'iL',num:'INT-S8',titre:'Dératisation Affectersonde',techId:'',techIds:[]})); }`,
    geste: `openSearch(); ${ATT(100)} renderSearch('Affectersonde'); window.__vu=[...document.querySelectorAll('#gsearch-res .pl-row')].filter(x=>x.textContent.includes('Affectersonde')).length;`,
    mesure: `window.__vu` },
  { nom: '   ⛔ … mais le CLIENT de cette intervention n’entre pas dans sa liste de clients, ni son registre', type: 'unique', attendu: 'bloque', retire: { voirTout: false },
    base: `function(){ db.interventions.push(Object.assign({},db.interventions.find(x=>x.id==='iZ'),{id:'iL',num:'INT-S8',titre:'Dératisation Affectersonde',techId:'',techIds:[]})); }`,
    geste: `window.__vu=(visibleClients(db.clients).some(c=>c.id==='cZ')?1:0)+(visibleDocs(db.devis).some(d=>d.clientId==='cZ')?1:0); registreClient=''; go('registre'); ${ATT(700)} window.__vu+=[...document.querySelectorAll('#content select option')].some(o=>o.textContent.includes('Zoésonde'))?1:0;`,
    mesure: `window.__vu` },
